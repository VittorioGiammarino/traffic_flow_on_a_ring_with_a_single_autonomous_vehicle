function dydt = ODE_Non_linear_simulation_general_new_AV_first_order(t,y,N,a,b,Ring_length,V,alpha,k,c,t_a,t_b,t_c)
%{
INPUTS:
...
switch nargin 
    ...
    case 10
        for i=1:2:(2*(N-1)-1)
          if (t>=560) && (t<=561)
          dydt(i,1)= y(i+1);
          dydt(i+1,1)= a*((y(i+3) - y(i+1))/(y(i+2) - y(i))^2) + b*(V(y(i+2)-y(i))-y(i+1)) - 1;
          else
          dydt(i,1)= y(i+1);
          dydt(i+1,1)= a*((y(i+3) - y(i+1))/(y(i+2) - y(i))^2) + b*(V(y(i+2)-y(i))-y(i+1));
          end
        ...
        if (t>=60) && (t<=61)
         dydt(2*N-1,1)= y(2*N);
         dydt(2*N,1) = k*(y(2)-y(2*N)) + c*(v_eq - y(2*N)) - 1;% - b*y(2*N);
        else
          dydt(2*N-1,1)= y(2*N);
          dydt(2*N,1) = k*(y(2)-y(2*N)) + c*(v_eq - y(2*N));% - b*y(2*N);
        end
    case 11
        if t <= t_a
            for i=1:2:(2*(N-1)-1)
