 function dydt = ODE_Linear_simulation_general_second_order_AV(t,y,N,a,b,Ring_length,V,k_bar,alpha,k_veh,beta,t_a,t_b,t_c)
%{
INPUTS:
...
switch nargin 
    case 8
        for i=1:2:(2*(N-1)-1)
          dydt(i,1)= y(i+1);
          dydt(i+1,1)= a*((y(i+3) - y(i+1))/(y(i+2) - y(i))^2) + b*(k_bar*(y(i+2)-y(i))+c-y(i+1));
        end
          dydt(2*N-1,1)= y(2*N);
          dydt(2*N,1) = a*(y(2) - y(2*N)) + b*(k_bar*(Ring_length+y(1)-y(2*N-1))+c-y(2*N));
    case 11
        for i=1:2:(2*(N-1)-1)
          dydt(i,1)= y(i+1);
          dydt(i+1,1)= a*(y(i+3) - y(i+1)) + b*(k_bar*(y(i+2)-y(i))+c-y(i+1));
        end
          dydt(2*N-1,1)= y(2*N);
          dydt(2*N,1) = k_veh*((1-alpha)*y(2)+alpha*(y(2*N)+y(2))/2 +(alpha*beta)*(y(1)-y(2*N-1)-7)/5-y(2*N));
    case 12
        if t <= t_a
            for i=1:2:(2*(N-1)-1)
                dydt(i,1)= y(i+1);
