function dydt = ODE_Non_linear_simulation_more_communication_online_Epos(t,y,N,a,b,Ring_length,V,alpha,k_veh,N_c,t_a,t_b,t_c)
%{
INPUTS:
...
switch nargin 
    ...
    case 10
        for i=1:2:(2*(N-1)-1)
          ...
          displacement_AV = y(2*N-1);
          displacement_AV=displacement_AV-(Ring_length*(floor(displacement_AV/Ring_length)+1));
          k = 0;
          for j=1:2:(2*N-1)
            if displacement_AV>y(j)
                k = k+1; %number of vehicles overtaken
            end
            if displacement_AV<y(j)
                break
            end
          ...
          v = 0;
          for i=(2+2*k):2:(2*N_c+2*k)
              v = v + y(i);
          end
          dydt(2*N,1) = k_veh*((1-alpha)*y(2*N_c+2*k)+alpha*(y(2*N)+v)/(N_c+1) - y(2*N));
    case 11
        if t <= t_a
            for i=1:2:(2*(N-1)-1)
                dydt(i,1)= y(i+1);
