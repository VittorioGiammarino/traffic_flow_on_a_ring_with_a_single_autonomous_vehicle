function dydt = ODE_Non_linear_simulation_general_PrePrint(t,y,N,a,b,Ring_length,V,beta,gamma,t_a,t_b,t_c)
%{
INPUTS:
...
switch nargin 
    ...
    case 9
            ...
            if (t>20) && (t<21)
                ...
                dydt(2*N,1) = beta*(alpha*v_target+(1-alpha)*y(2)-y(2*N)) + 0.5*(v_eq-y(2*N))-0.05;%-5; % -0.05 disturb final thesis
            else
                %if (Ring_length+y(1)-y(2*N-1)) > 6
                dydt(2*N-1,1)= y(2*N);
                Vd=(y(2)+y(2*N))/2;
                ...
                alpha = min(max(((Ring_length+y(1)-y(2*N-1)-Delta_x_s)/gamma),0),1);
                dydt(2*N,1) = beta*(alpha*v_target+(1-alpha)*y(2)-y(2*N)) + 0.5*(v_eq-y(2*N));
            end
