function dydt = ODE_Non_linear_simulation_general_more_communication(t,y,N,a,b,Ring_length,V,alpha,k_veh,N_c,t_a,t_b,t_c)
%{
INPUTS:
...
switch nargin 
    ...
    case 10
        for i=1:2:(2*(N-1)-1)
          ...
          dydt(2*N,1) = k_veh*((1-alpha)*y(2)+alpha*(y(2*N)+v)/(N_c+1) - y(2*N));
    case 11
        if t <= t_a
            for i=1:2:(2*(N-1)-1)
                dydt(i,1)= y(i+1);
