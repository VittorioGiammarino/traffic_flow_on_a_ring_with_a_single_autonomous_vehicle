function dydt = ODE_Non_linear_simulation_general(t,y,N,a,b,Ring_length,V,alpha,k_veh,t_a,t_b,t_c)
%{
INPUTS:
...
switch nargin 
    case 7
        ...
        if (t>10)&&(t<13)
          dydt(2*N-1,1)= y(2*N);
          dydt(2*N,1) = a*((y(2) - y(2*N))/(Ring_length+y(1) - y(2*N-1))^2) + b*(V(Ring_length+y(1)-y(2*N-1))-y(2*N)) - 1;
        else
          dydt(2*N-1,1)= y(2*N);
