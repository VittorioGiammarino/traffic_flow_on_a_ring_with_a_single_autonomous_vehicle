...
xlim([-1 0.5])
ylim([-1.5 1.5])
legend('poles of F^{(22)}_i','poles of F^{AV(22)}_i with AV in [16]')
...
xlim([-1 0.5])
ylim([-1.5 1.5])
legend('poles of F^{(22)}_i','poles of F^{AV(22)}_i with proposed AV')
...
[t,y_N_cars] = ode45(@(t,y) ODE_Non_linear_simulation_general_PrePrint(t,y,N,a,b,Ring_length,V,k_veh2,2),[0 t_f],y0,opts);
...
% end
% 
% video = VideoWriter('Modified_PI_with_noise.avi', 'Motion JPEG AVI');
% %video.FrameRate = 10;
% open(video)
% writeVideo(video, F);
% close(video)
